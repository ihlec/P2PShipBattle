...
Browser co-op pirate sandbox. Vanilla JS, Canvas 2D, P2P via Trystero. Host runs world/NPC authority; clients sync to the host seed and entity state. Graphics are drawn in code (no asset packs).
...
* **`src/config.js`:** Tile IDs, colors, stats, rules.
* **`src/utils.js`:** Noise, hash, lerp, distance helpers.
* **`src/core/`**:
    * `Game.js`: Main loop and game state.
    * `Network.js`: P2P sync.
    * `InputHandler.js`: Keyboard / mouse / touch.
* **`src/world/`**:
